class BarChart {
    ...
    generateBars(idFilter1, idFilter2, dataPath = '../static/data/df_foxes_with_ID.csv') {
        d3.csv(dataPath)
            .then((data) => {
                ...
                const barSpacing = 40;
                const offsetY = 80;
                const chartSpacing = this.width / 2  +180;
                ...
                // Draw both charts
                this.drawChart(combinedGeometric, 0, 'geometric');
                this.drawChart(combinedKinematic, chartSpacing, 'kinematic');
    ...
    drawChart(data, xOffset, className) {
        const barHeight = 20;
        const barSpacing = 40;
        const offsetY = 80;
        const widthScale = d3.scaleLinear()
        ...
        this.svg.selectAll(`.bar1-${className}`)
            .data(data)
            .join('rect')
        ...
        this.svg.selectAll(`.bar2-${className}`)
            .data(data)
            .join('rect')
        ...
        // Feature labels
        this.svg.selectAll(`.label0-${className}`)
            .data(data)
            .join('text')
        ...
        this.svg.selectAll(`.label1-${className}`)
            .data(data)
            .join('text')
        ...
        this.svg.selectAll(`.label2-${className}`)
            .data(data)
            .join('text')
